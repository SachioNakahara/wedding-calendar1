...
const oauth2Client = new google.auth.OAuth2(
  ...
  `${process.env.GOOGLE_REDIRECT_URI}/auth/callback` // 修正: 正しいコールバックURLに統一
);
...
app.get("/auth/callback", async (req, res) => {
  ...
  if (!code) {
    ...
    return res.redirect(`${process.env.CLIENT_ORIGIN}?error=no_code`);
  }

  // stateパラメータの検証（セキュリティ向上）
  if (!state || !state.startsWith("security_token_")) {
    console.error("不正なstateパラメータ:", state);
    return res.redirect(`${process.env.CLIENT_ORIGIN}?error=invalid_state`);
  }
  ...
  try {
    ...
    res.redirect(`${process.env.CLIENT_ORIGIN}?error=token_failed`);
  }
});
...
app.get("/api/config", (req, res) => {
  res.json({
    apiKey: process.env.GOOGLE_API_KEY ? "設定済み" : "未設定",
...
// サーバー起動
app.listen(PORT, () => {
  console.log(`🚀 ウエディングカレンダー連携サーバーが起動しました`);
  console.log(`📅 ポート: ${PORT}`);
  console.log(`🌐 環境: ${process.env.NODE_ENV || "development"}`);
  console.log(`🔗 クライアントURL: ${process.env.CLIENT_ORIGIN}`);
  console.log(
    `🔑 Google Client ID: ${
  ...
  console.log(
    `🔑 Google API Key: ${process.env.GOOGLE_API_KEY ? "設定済み" : "未設定"}`
  );
  console.log(
    `🔗 リダイレクトURI: ${process.env.GOOGLE_REDIRECT_URI}/auth/callback`
  );

  // 重要な設定の確認
  if (!process.env.GOOGLE_CLIENT_ID) {
    console.warn("⚠️  GOOGLE_CLIENT_ID が設定されていません");
  }
  if (!process.env.GOOGLE_CLIENT_SECRET) {
    console.warn("⚠️  GOOGLE_CLIENT_SECRET が設定されていません");
  }
  if (
    !process.env.SESSION_SECRET ||
    process.env.SESSION_SECRET === "your-secret-key"
  ) {
    console.warn("⚠️  SESSION_SECRET を本番用に変更してください");
  }
  ...
  if (process.env.DEBUG === "true") {
    ...
    console.log("   GET  /auth/google - Google認証URL取得");
    console.log("   GET  /auth/callback - OAuth2コールバック");
    console.log("   GET  /auth/status - 認証状態確認");
    console.log("   POST /auth/logout - ログアウト");
    console.log("   GET  /api/config - API設定情報");
    console.log("   GET  /api/calendars - カレンダー一覧");
    console.log("   GET  /api/events - イベント一覧");
    console.log("   POST /api/events - イベント作成");
    console.log("   PUT  /api/events/:id - イベント更新");
    console.log("   DELETE /api/events/:id - イベント削除");
    console.log("   POST /api/sync - 一括同期");
  }
});
